      ...
      for t in s.vpc[*] : [
        for u in t.vswitch[*] : [
          for v in u.nat_gateway : {
            ...
            eip_high_definition_monitor_log_status = lookup(v, "eip_high_definition_monitor_log_status", "OFF")
            eip_internet_charge_type               = lookup(v, "eip_internet_charge_type", "PayByTraffic")
